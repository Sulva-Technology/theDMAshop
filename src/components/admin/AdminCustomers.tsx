...
export default function AdminCustomers() {
  ...
  const { customers, customersLoading, customersError } = useStore();
  ...
  const selectedCustomer = filteredCustomers.find((customer) => customer.id === selectedCustomerId) ?? customers.find((customer) => customer.id === selectedCustomerId) ?? null;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border/50">
              ...
              {!customersLoading && customersError && (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-red-600">{customersError}</td>
                </tr>
              )}
      ...
      <Sheet open={Boolean(selectedCustomer)} onOpenChange={(open) => !open && setSelectedCustomerId(null)}>
