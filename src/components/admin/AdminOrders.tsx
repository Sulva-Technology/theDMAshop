...
import { Eye, Filter, MapPin, Search, Truck, CheckCircle, XCircle } from 'lucide-react';
...
export default function AdminOrders() {
  ...
  const handleStatusAdvance = async (order: Order) => {
    ...
    if (selectedOrder?.id === order.id) {
      setSelectedOrder({ ...order, fulfillmentStatus: nextStatus, status: nextStatus });
    }
  };
  ...
  const handleCancel = async (order: Order) => {
    ...
    if (selectedOrder?.id === order.id) {
      setSelectedOrder({ ...order, fulfillmentStatus: 'cancelled', status: 'cancelled' });
    }
  };
